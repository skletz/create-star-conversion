...
float getMinAngleError(const float angle1, const float angle2, const bool fast) {
  if(fast) {
    ...
    return min(angleError1, angleError2);
  } else {
...
float getMinAngleError(const float angle1, const float angle2, const bool degree, const bool customPolarAngle) {
  if(degree) {
    if(customPolarAngle) {
      return min( 180 - fabs(angle1-angle2), fabs(angle1-angle2) );
    } else {
      return min( 360 - fabs(angle1-angle2), fabs(angle1-angle2) );
    }
  } else {
    if(customPolarAngle) {
      return min( M_PI - fabs(angle1-angle2), fabs(angle1-angle2) );
    } else {
      return min( 2.0*M_PI - fabs(angle1-angle2), fabs(angle1-angle2) );
    }
  }
}

...
static vector<Point> getSimpleContours(const Mat& currentQuery, int points = NRPOINTS)
{
...
int main(int argc, char** argv)
{
	...
	string path = "../../../data/sketch_sample";
    ...
    {
        std::stringstream dbDir;
        dbDir << path_images << i << "/";
		boost::filesystem::path p(dbDir.str());

		std::vector<std::string> files;
		//for(auto & p : std::experimental::filesystem::directory_iterator(dbDir.str()))
		//{
		//	if(counter > 0)
		//	{
		//		std::string file = std::string(p.path().string());
		//		size_t pos = file.find(".jpg");
		//		    
		//		if(pos != std::string::npos){
		//		    files.push_back(std::string(p.path().string()));
		//		    counter--;
		//		}
		//	}
		//		
		...
		for (boost::filesystem::directory_iterator itr(p); itr != end_itr; ++itr)
		{
			...
			if (boost::filesystem::is_regular_file(itr->path())) {
				...
				if(counter > 0)
				{
					...
					if(boost::filesystem::extension(itr->path().string()) == ".jpg"){

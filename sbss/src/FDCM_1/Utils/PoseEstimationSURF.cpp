  ...
  , draw_type_(CV_AA) // anti-aliasing drawing (CV_AA is 16), about 2ms more required
{
  ...
  surfDetector_ = cv::xfeatures2d::SURF::create(surf_min_hessian_);
}
...
// https://stackoverflow.com/questions/6751981/converting-a-vector-of-keypoints-to-a-cvseq
void CPoseEstimationSURF::copyKeypointVectorToSeq(vector<cv::KeyPoint> &sourceKeypoints, CvSeq *targetSeq)
{
  ...
  targetSeq = cvCreateSeq(0,sizeof(CvSeq),sizeof(KeyPoint),storage);
  // Create the seq at the location storage

  for (size_t i=0; sourceKeypoints.size(); i++) {
    int* added = (int*)cvSeqPush(targetSeq,&(sourceKeypoints[i]));
...
CvMat* CPoseEstimationSURF::estimatePose(int &num_of_corr)
{
  ...
  cv::Mat inputImg = cv::cvarrToMat(img_input_);
  surfDetector_->compute(inputImg, keypoints, descriptors);

  // vector to cvSeq
  copyKeypointVectorToSeq(keypoints, seq_keypoints_);
  copyKeypointVectorToSeq(descriptors, seq_descriptors_);

  // old opencv2
  //extractSURF(img_input_, 0, &seq_keypoints_, &seq_descriptors_, ms_, surf_params_);
...
void CPoseEstimationSURF::findCorrespondenceNN_FLANN(const CvSeq* imageKeypoints, const CvSeq* imageDescriptors, vector<int>& ptpairs, int numOfKeyframes)
{
  ...
  for(int i = 0; i < imageDescriptors->total; i++ )
  {
    ...
    const KeyPoint* kp = (const KeyPoint*)kreader.ptr;

...
void CObjectModel::copyKeypointVectorToSeq(vector<cv::KeyPoint> &sourceKeypoints, CvSeq *targetSeq)
{
  ...
  targetSeq = cvCreateSeq(0,sizeof(CvSeq),sizeof(cv::KeyPoint),storage);
...
void CObjectModel::_saveSURFkeypoints(std::string& obj_name, IplImage* imgG, CvMat* pose, vector<CvPoint2D32f>& new2Dpt, vector<vector<float> >& newDesc, vector<CvPoint3D32f>& new3Dpt)
{
  ...
  cv::Ptr<cv::xfeatures2d::SURF> surfDetector = cv::xfeatures2d::SURF::create(100);

  vector<cv::KeyPoint> keypoints;
  vector<cv::KeyPoint> descriptors;

  // Extract SURF features on test images.
  cv::Mat inputImg = cv::cvarrToMat(imgG);
  surfDetector->compute(inputImg, keypoints, descriptors);

  // vector to cvSeq
  copyKeypointVectorToSeq(keypoints, imageKeypoints);
  copyKeypointVectorToSeq(descriptors, imageDescriptors);
  ...
  for(int i=0; i<imageDescriptors->total; i++ )
  {
    ...
    const cv::KeyPoint* kp = (const cv::KeyPoint*)kreader.ptr;
